...
<div class="container py-4">
    <div class="d-flex align-items-center mb-4 gap-2">
        <h2 class="mb-0">Tổng hợp yêu cầu hỗ trợ</h2>
    </div>
            ...
            <tbody>
                @forelse($requests as $req)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('support.show', $req) }}" class="btn btn-outline-primary btn-sm rounded-pill px-3">Xem chi tiết</a>
                            @if($req->status == 'open')
                                <form action="{{ route('admin.support.close', $req) }}" method="POST" class="d-inline ms-1">
                                    @csrf
                                    <button type="submit" class="btn btn-warning btn-sm rounded-pill px-3" title="Đóng yêu cầu">Đóng</button>
                                </form>
                            @endif
                            <form action="{{ route('admin.support.destroy', $req) }}" method="POST" class="d-inline form-delete ms-1">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger btn-sm rounded-pill px-3" title="Xóa">Xóa</button>
                            </form>
                        </td>
                    </tr>
                @empty
